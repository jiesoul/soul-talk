...
(defn insert-article! [article]
  (let [now (utils/now)
        id  (id-util/generate-id!)
        _ (log/debug "id: " id)
        article (article-db/save-article!
                  (assoc article :id id :create_at now :update_at now :publish publish-false))]
...
(defn update-article! [article]
  (let [id (:id article)
        _ (article-db/update-article! (assoc article :update_at (utils/now) :publish publish-false))
        article (article-db/get-article-by-id id)]
    (utils/ok {:article article})))

(defn publish-article! [id]
  (if-let [article (article-db/get-article-by-id id)]
    (let [article (assoc article :publish publish-true :update_at (utils/now))
          _ (article-db/publish-article! article)]
